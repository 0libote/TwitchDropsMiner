...
class History:
    def __init__(self, path: str | Path) -> None:
        ...
        if legacy:
            with self.db:
                self.db.execute(
                    "ALTER TABLE claims RENAME COLUMN claimed_at TO recorded_at"
                )
    ...
    def ingest_inventory(self, user_id: str, game_event_drops: Any) -> None:
        ...
        with self.db:
            ...
            self.db.execute(
                "INSERT INTO syncs VALUES (?,?) ON CONFLICT(account_id) "
        ...
        for campaign in campaigns:
            for drop in campaign.drops:
                for benefit in drop.benefits:
                    matches.setdefault(str(benefit.id), []).append(
                        (campaign, drop, benefit)
                    )
        with self.db:
            for row in self.db.execute(
                "SELECT * FROM rewards WHERE account_id=?", (str(user_id),)
            ).fetchall():
                candidates = matches.get(row["benefit_id"], [])
                awarded = row["last_awarded_at"]
                candidates = [
                    (c, d, b)
                    for c, d, b in candidates
                    if awarded
                ...
                c, _, b = candidates[0]
                self.db.execute(
                    """UPDATE rewards SET name=COALESCE(name,?),
        ...
        with self.db:
            inserted = self.db.execute(
                ...
                (
                    ...
                    drop.id,
                    now,
                    str(campaign.game.id),
                    campaign.game.name,
                    campaign.name,
                    drop.name,
                    json.dumps([str(b.id) for b in drop.benefits]),
                ),
            ).rowcount
            if not inserted:
                return
            for b in drop.benefits:
                self._reward(
                    ...
                    str(b.id),
                    name=b.name,
                    image=str(b.image_url),
                    game_id=str(campaign.game.id),
                    game_name=campaign.game.name,
                    source="local",
                )
                self.db.execute(
                    "UPDATE rewards SET campaign_id=?,campaign_name=? "
                    "WHERE account_id=? AND benefit_id=?",
    ...
    def query(
        ...
        if search:
            where += (
                " AND (instr(lower(COALESCE(name,'')),lower(?))>0 OR "
        ...
        total = self.db.execute(
            f"SELECT COUNT(*) FROM rewards WHERE {where}", values
        ).fetchone()[0]
        rows = self.db.execute(
            f"SELECT * FROM rewards WHERE {where} "
        ...
        items = [
            {
                ...
                "name": r["name"] or "Unknown reward",
                "imageUrl": r["image_url"],
                "gameId": r["game_id"],
                "gameName": r["game_name"] or "Unknown game",
                "campaignId": r["campaign_id"],
                "campaignName": r["campaign_name"],
                "lastAwardedAt": r["last_awarded_at"],
    ...
    def summary(self, user_id: str) -> dict[str, Any]:
        ...
        count, first = self.db.execute(
            "SELECT COUNT(*),MIN(observed_at) FROM rewards WHERE account_id=?", account
        ).fetchone()
        games = [
            {"id": r[0], "name": r[1] or "Unknown game", "rewardCount": r[2]}
            for r in self.db.execute(
                "SELECT game_id,MAX(game_name),COUNT(*) FROM rewards "
                "WHERE account_id=? GROUP BY game_id ORDER BY 2",
                account,
            )
        ]
        local_by_game = {
            r[0]: r[1]
            for r in self.db.execute(
                "SELECT game_id,COUNT(*) FROM claims WHERE account_id=? GROUP BY game_id",
                account,
            )
        }
        for game in games:
        ...
        sync = self.db.execute(
            "SELECT synced_at FROM syncs WHERE account_id=?", account
        ).fetchone()
        return {
            "rewardCount": count,
            "localClaimCount": self.db.execute(
                "SELECT COUNT(*) FROM claims WHERE account_id=?", account
            ).fetchone()[0],
            "gameCount": sum(g["id"] is not None for g in games),
            "games": games,
            "firstObservedAt": first,
            "lastSyncedAt": sync[0] if sync else None,
            "coverage": COVERAGE,
            "dailyClaims": daily,
        }
