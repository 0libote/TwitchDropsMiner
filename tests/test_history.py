...
class HistoryTests(unittest.TestCase):
    ...
    def test_inventory_is_idempotent_persistent_and_account_scoped(self):
        ...
        self.assertNotIn(
            "not stored",
            self.history.db.execute("SELECT metadata FROM rewards").fetchone()[0],
        )
        self.history.close()
        self.history = History(self.path)
        self.assertEqual(
            self.history.query("a")["items"][0]["observedAt"], first["observedAt"]
        )

    def test_tolerates_missing_metadata_and_literal_search(self):
        self.history.ingest_inventory(
            "a",
            [
                None,
                {},
                {"id": "1", "lastAwardedAt": "invalid"},
    ...
    def test_claim_and_inventory_do_not_double_count_rewards(self):
        campaign = SimpleNamespace(
            id="c", name="Campaign", game=SimpleNamespace(id="g", name="Game")
        )
        benefit = SimpleNamespace(
            id="b", name="Reward", image_url="https://example.test/a.png"
        )
        drop = SimpleNamespace(
            id="d", name="Drop", campaign=campaign, benefits=[benefit]
        )
        ...
        self.assertEqual(
            sum(d["count"] for d in self.history.summary("b")["dailyClaims"]), 0
        )
        ...
        self.history.record_claim("a", drop)
        self.assertEqual(
            self.history.query("a")["items"][0]["lastAwardedAt"], confirmed
        )
        old = (datetime.now(timezone.utc) - timedelta(days=35)).isoformat()
        self.history.db.execute(
            "UPDATE claims SET recorded_at=? WHERE drop_id=?", (old, drop.id)
        )
        self.assertEqual(
            sum(d["count"] for d in self.history.summary("a")["dailyClaims"]), 1
        )

    def test_future_and_foreign_schemas_rejected_without_writes(self):
        self.assertEqual(
            self.history.db.execute("PRAGMA user_version").fetchone()[0], 1
        )
    ...
    def test_initial_unversioned_schema_migrates_without_losing_data(self):
        ...
        self.history.db.execute(
            "ALTER TABLE claims RENAME COLUMN recorded_at TO claimed_at"
        )
    ...
    def test_enrichment_requires_unambiguous_matching_award_window(self):
        ...
        campaign = SimpleNamespace(
            id="c",
            name="Campaign",
            game=SimpleNamespace(id="g", name="Game"),
            drops=[drop],
        )
        self.history.ingest_inventory(
            "a", [{"id": "b", "lastAwardedAt": "2024-01-02T00:00:00Z"}]
        )
