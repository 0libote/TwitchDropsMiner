...
class MiningPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.miner = SimpleNamespace(
            wanted_games=[self.game],
            inventory=[],
            channels={1: self.channel},
            watching_channel=AwaitableValue(),
            paused=False,
            settings=SimpleNamespace(priority=[], exclude=set()),
            can_watch=lambda _: True,
            get_active_campaign=Mock(return_value=None),
        )
    ...
    def test_other_campaign_live_channel_does_not_make_blocked_campaign_live(
        self,
    ) -> None:
    ...
    def test_current_fallback_precedes_unavailable_priority_without_eta_delay(
        self,
    ) -> None:
...
class WatchdogTests(unittest.IsolatedAsyncioTestCase):
    async def run_ticks(self, elapsed):
        miner = SimpleNamespace(
            seconds_without_progress=Mock(return_value=elapsed), change_state=Mock()
        )
        ui = SimpleNamespace(
            _twitch=miner,
            last_watchdog=0.0,
        ...
        with (
            patch("webui.monotonic", side_effect=[1000, 1060, 1120]),
            patch(
                "webui.asyncio.sleep",
